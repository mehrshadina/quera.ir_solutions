...
class Program:
    ...
    def add_new_car(self, brand, model, color, purchase_price):
        car = Mobile(self.next_id, brand, model, color, purchase_price)
        self.cars.append(car)
        self.next_id += 1
        print("New car is successfully added.")

    def tasmim(self, command):
        parts = command.split()
        vorodi = ' '.join(parts[:3])
        if vorodi == "Help":
            print(
        ...
        elif vorodi == "Purchase new car":
            self.add_new_car(parts[3].strip(),
                              parts[4].strip(), parts[5].strip(), 
                              int(parts[6].strip()))
        elif vorodi == "Purchase used car":
            self.add_used_car(parts[3].strip(),
                               parts[4].strip(), parts[5].strip(),
                                 int(parts[6].strip()), int(parts[7].strip()), int(parts[8].strip()))
        elif vorodi.startswith("Sale car"):
            self.sale_car(int(parts[2].strip()), 
                          int(parts[3].strip()), 
                          parts[4].strip())
        elif vorodi == "Show contracts":
            self.show_contracts()
        elif vorodi == "Add maintenance":
            ...
            self.add_used_car(parts[3].strip(), parts[4].strip(), 
                              parts[5].strip(), int(parts[6].strip()), int(parts[7].strip()), int(parts[8].strip()))
       
        elif vorodi.startswith("Sale car"):
            self.sale_car(int(parts[2].strip()), int(parts[3].strip()),
                           parts[4].strip())
            ...
            if car:
                ...
                print("Car with this ID is not found!")
        elif vorodi == "Show available cars":
            self.show_available()

        elif vorodi == "Show available new":
            self.show_available_new_cars()

        elif vorodi == "Show available used":
    ...
    def add_used_car(self, brand, model, color, 
                     purchase_price, used_years, used_km):
        car = Mobile(self.next_id, brand, model, color, 
                     purchase_price, used_years, used_km)
        self.cars.append(car)
